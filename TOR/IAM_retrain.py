...
def create_image_list_all(image_dir, testing_number, validation_number):
  ...
  for sub_dir in sub_dirs:
    ...
    for file_name in file_list:
      ...
      """
      training_images.append(base_name)
      if len(validation_images) < validation_number:
        validation_images.append(base_name)
      if len(testing_images) < testing_number:
        testing_images.append(base_name)
...
def main(_):
  ...
  if FLAGS.training_number == "all":
    image_lists = create_image_list_all(FLAGS.image_dir,
                                        FLAGS.testing_number,
                                        FLAGS.validation_number)
  else:  
    # Look at the folder structure, and create lists of all the images.
    image_lists = create_image_lists(FLAGS.image_dir, int(FLAGS.training_number),
                                     FLAGS.testing_number,
                                     FLAGS.validation_number)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      type=str,
      default="all",
      help='What number of images to use as a train set.'
  )
  parser.add_argument(
      '--validation_number',
      type=int,
      default=1,
      help='What number of images to use as a validation set.'
  )
  parser.add_argument(
      '--testing_number',
      type=int,
      default=1,
      help='What number of images to use as a test set.'
  )
  ###
  parser.add_argument(
      '--testing_percentage',
  ...
  parser.add_argument(
      '--using_Adam',
      default=False,
      help='Flag to use Adam optimizer',
      action='store_true')
